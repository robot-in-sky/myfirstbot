...
from myfirstbot.db import BaseM


TModel = TypeVar("TModel", bound=BaseM)


class AbstractOrmRepo(Generic[TModel]):

    def __init__(self, session: AsyncSession, model: BaseM) -> None:
        # super().__init__(session)
        self.session = session
        self.model = model

    async def add(self, **attrs: Any) -> TModel:
        query = insert(self.model).values(**attrs).returning(self.model)
    ...
    async def update(self, ident: int, **attrs: Any) -> TModel | None:
        query = update()
